...
if [ $1 = "part3_1" ]; then
  condor_submit transfer_input_files=bin/${1},bin/part3_reference1,bin/part3_reference2,src/part3/run.py,src/part3/graph.py transfer_output_files=data,graphs executable=scripts/${1}.sh log=${1}.log error=${1}.error output=${1}.out launch.condor
else
